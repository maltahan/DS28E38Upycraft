...
class Main: 
    def connect_to_ds28e38(self):
        while True:                                      
            ...
            #the crc for the sequence of commands to get the signature crc checksum
            ...
            try:
              ...
              if(exp and counter < 3):
                ...
                print("the exception is still exists. the details is bellow2:")
                raise Exception(exp)  
            ...
            complete_data['crc16_res'] = signature_data['crc16_res']
            complete_data['crc16_cal'] = signature_data['crc16_cal']
            ...
            if(signature_data['crc16_res'] == signature_data['crc16_cal']):
              conn.sendall('HTTP/1.1 200 OK\nConnection: close\nServer: FireBeetle\nContent-Type: application/json\nAccess-Control-Allow-Origin:*\nContent-length:{:d}\n\n'.format(len(signature_data_json)))
              conn.sendall(signature_data_json)
              crc_counter = 0
              print(signature_data_json)
            elif(signature_data['crc16_res'] != signature_data['crc16_cal'] and crc_counter < 5):
              retry = {}
              retry['message'] = 'false'
              retry_data = json.dumps(retry)
              conn.sendall('HTTP/1.1 200 OK\nConnection: close\nServer: FireBeetle\nContent-Type: application/json\nAccess-Control-Allow-Origin:*\nContent-length:{:d}\n\n'.format(len(retry_data)))
              conn.sendall(retry_data)
              crc_counter+=1
            else:
              error_msg = {}
              error_msg['message'] = 'we still get unvalid signature'
              error_data = json.dumps(error_msg)
              conn.sendall('HTTP/1.1 400  Invalid signature\nConnection: close\nServer: FireBeetle\nContent-Type: application/json\nAccess-Control-Allow-Origin:*\nContent-length:{:d}\n\n'.format(len(error_data)))
...
try:
    ...
    obj.connect_to_ds28e38()
     ...
     if (exp and counter < 3):
        ...
        print("the exception is still exists. the details is bellow:")
        raise Exception(exp) 
